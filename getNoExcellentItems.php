...
$stmt = $conn->prepare('SELECT username FROM users WHERE username NOT IN (SELECT items.username FROM items WHERE title IN (SELECT itemName FROM reviews WHERE rating = "excellent" GROUP BY itemName HAVING COUNT(*) >= 3))');
